...
def move_with_retry(src, dst, attempts=5, wait=2):
    for attempt in range(attempts):
        ...
        print(f"Failed to move file after {attempts} attempts.")
...
class Workspace:
    ...
    def train_bc(self):
        ...
        while train_until_step(self.global_step): # 
            # try to evaluate
            if eval_every_step(self.global_step): #self.global_step
                self.logger_bc.log(
        ...
        move_with_retry(self.work_dir / "train.csv", self.work_dir / "train_bc.csv")
        move_with_retry(self.work_dir / "eval.csv", self.work_dir / "eval_bc.csv")
        move_with_retry(self.work_dir / "eval_video", self.work_dir / "eval_video_bc")
